...
int16_t tmsK1[0x20]     =
	{
	0x82C0,0x8380,0x83C0,0x8440,0x84C0,0x8540,0x8600,0x8780,0x8880,0x8980,0x8AC0,0x8C00,0x8D40,0x8F00,0x90C0,0x92C0,0x9900,0xA140,0xAB80,0xB840,0xC740,0xD8C0,0xEBC0,0x0000,0x1440,0x2740,0x38C0,0x47C0,0x5480,0x5EC0,0x6700,0x6D40
	};
int16_t tmsK2[0x20]     =
	{
	0xAE00,0xB480,0xBB80,0xC340,0xCB80,0xD440,0xDDC0,0xE780,0xF180,0xFBC0,0x0600,0x1040,0x1A40,0x2400,0x2D40,0x3600,0x3E40,0x45C0,0x4CC0,0x5300,0x5880,0x5DC0,0x6240,0x6640,0x69C0,0x6CC0,0x6F80,0x71C0,0x73C0,0x7580,0x7700,0x7E80
	};
int8_t tmsK3[0x10]      =
	{
...
void Talkie::BendingOn()
{
 Talkie::Talkie_bend=true;
}

void Talkie::BendingOff()
{
 Talkie::Talkie_bend=false;
}

void Talkie::SetBending(int bending)
{
 Talkie::Talkie_bending=bending;
}

void Talkie::SetSpeed(int speed)
{
 Talkie::Talkie_speed=speed;
}

void Talkie::SetPitch(int pitch)
{
 Talkie::Talkie_pitch=pitch;
}
...
void Talkie::say(const uint8_t *addr)
	...
	do
		...
		// PITCH HACK
		long val = map(Talkie::Talkie_pitch, 0, 1023, 1000, 10000); //was 10000
		...
		if(energy == 0)
			...
			if(!repeat)
				...
				if(synthPeriod)
					...
					if(Talkie::Talkie_bend)  /// sorry guys bending the neck here !
						{
						synthK5= map(Talkie::Talkie_bending,20,1000,0,200);
						synthK10= map(Talkie::Talkie_bending,20,1000,200,0);
						}
					}
				}
			}
		...
		//SPEED HACK
		delay( map(Talkie::Talkie_speed, 0, 1023, 100, 0)); //speed
		}
	while(energy != 0xf);
	}
